...
class mission_fence_point_s:
    ...
    def getItem(self):
        if self.nav_cmd == NAV_CMD.FENCE_POLYGON_VERTEX_INCLUSION.value or self.nav_cmd == NAV_CMD.FENCE_POLYGON_VERTEX_EXCLUSION.value:
            return [self.lat, self.lon, self.alt, self.vertex_count, self.nav_cmd, self.frame]
        elif self.nav_cmd == NAV_CMD.FENCE_CIRCLE_INCLUSION.value or self.nav_cmd == NAV_CMD.FENCE_CIRCLE_EXCLUSION_EXCLUSION.value:
            return [self.lat, self.lon, self.alt, self.circle_radius, self.nav_cmd, self.frame]


class mission_item_s:
    def __init__(self, _lat, _lon, _time_inside, _circle_radius, _acceptance_radius, _loiter_radius, _yaw, _altitude,
                 _params, _nav_cmd, _do_jump_mission_index, _do_jump_repeat_count, _union1, _frame, _origin,
                 _loiter_exit_xtrack, _force_heading, _altitude_is_relative, _autocontinue, _vtol_back_transition):
        self.lat = _lat
        self.lon = _lon
    ...
    def getItem(self):
        res = [self.lat, self.lon]

        if self.frame == 2:
            res = res + self.params
        else:
            if self.nav_cmd == NAV_CMD.FENCE_POLYGON_VERTEX_INCLUSION.value or self.nav_cmd == NAV_CMD.FENCE_POLYGON_VERTEX_EXCLUSION.value\
                    or self.nav_cmd == NAV_CMD.FENCE_CIRCLE_INCLUSION.value or self.nav_cmd == NAV_CMD.FENCE_CIRCLE_EXCLUSION.value:
                res = res + [self.circle_radius, self.acceptance_radius, self.loiter_radius, self.yaw, self.altitude]

            else:
                res = res + [self.time_inside, self.acceptance_radius, self.loiter_radius, self.yaw, '-', '-',
                             self.altitude]

        res = res + [self.nav_cmd, self.do_jump_mission_index, self.do_jump_repeat_count, self.union, self.frame,
                     self.origin, self.loiter_exit_xtrack, self.force_heading, self.altitude_is_relative,
                     self.autocontinue, self.vtol_back_transition]
        print(res)
        return res
...
class mission_s:
    ...
    def getItem(self):
        return [self.timestamp, self.current_seq, self.count, self.dataman_id]
